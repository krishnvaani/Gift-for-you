// Final JS - admin/admin123, receiver/receiver123
...
siteTitle.addEventListener('blur', ()=>{ state.settings.title = siteTitle.textContent || "My Heart's Journey"; document.getElementById('pageTitle').textContent = state.settings.title; state.activity.push(Title changed by ${(currentUser?currentUser.name:'?')} at ${new Date().toLocaleString()}); save(); });
...
addContentBtn.addEventListener('click', async ()=>{
  ...
  state.activity.push(Admin added message \"${title||''}\" at ${new Date().toLocaleString()});
...
attachAudio.addEventListener('change', async (e)=>{ const f = e.target.files[0]; if(!f) return; const data = await fileToBase64(f); state.settings.audioData = data; bgPlayer.src = data; bgPlayer.play().catch(()=>{}); toggleAudioBtn.textContent='Pause'; state.activity.push(Admin set background audio at ${new Date().toLocaleString()}); save(); });
...
function startReminderChecker(){ setInterval(()=>{ const now=new Date(); const due = state.reminders.filter(r=> !r.notified && new Date(r.when) <= now); due.forEach(r=>{ r.notified=true; navigator.vibrate && navigator.vibrate(60); state.activity.push('Reminder fired: '+r.title+' at '+new Date().toLocaleString()); }); save(); },10000); }
