// Final script with admin features, seen tracking, comment history, and no auto-login
const STORAGE_KEY = 'gift_for_you_final_v2';
let state = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');
if(!state.settings){
  state = {
    settings: { title: 'Happy Birthday 💖', subtitle: 'Every day with you is a new chapter in my favorite story.', frontImage:'', audioData:null, credentials:{ adminUser:'admin', adminPass:'admin123', receiverUser:'receiver', receiverPass:'receiver123' } },
    messages: [],
    reminders: [],
    activity: [],
    nextId: 1
...
function lockBody(lock){ if(lock){ document.body.style.overflow='hidden'; } else { document.body.style.overflow='auto'; } }
...
closeLogin.addEventListener('click', ()=>{ loginModal.classList.add('hidden'); lockBody(false); });

// show app
function showApp(){
  loginModal.classList.add('hidden'); lockBody(false);
  app.classList.remove('hidden');
  userLabel.textContent = ${currentUser.name} (${currentUser.role});
  toggleAudioBtn.classList.remove('hidden'); logoutBtn.classList.remove('hidden');
...
logoutBtn.addEventListener('click', ()=>{ currentUser=null; localStorage.removeItem('gift_user'); location.reload(); });

// editable title/subtitle
siteTitle.addEventListener('blur', ()=>{ state.settings.title = siteTitle.textContent || 'Happy Birthday 💖'; document.getElementById('pageTitle').textContent = state.settings.title; state.activity.push(Title changed by ${currentUser?currentUser.name:'?'} at ${new Date().toLocaleString()}); save(); });
subtitle.addEventListener('blur', ()=>{ state.settings.subtitle = subtitle.textContent || ''; state.activity.push(Subtitle changed by ${currentUser?currentUser.name:'?'} at ${new Date().toLocaleString()}); save(); });

// heart image upload admin
heartImage.addEventListener('click', ()=>{ if(!currentUser || currentUser.role!=='admin') return; const ip=document.createElement('input'); ip.type='file'; ip.accept='image/*'; ip.onchange=(e)=>{ const f=e.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=(ev)=>{ state.settings.frontImage = ev.target.result; heartImage.src = ev.target.result; state.activity.push(Admin updated heart image at ${new Date().toLocaleDateString()}); save(); }; r.readAsDataURL(f); }; ip.click(); });
...
function showSection(name){ readView.classList.add('hidden'); archiveView.classList.add('hidden'); adminView.classList.add('hidden'); infoView.classList.add('hidden'); if(name==='read') readView.classList.remove('hidden'); if(name==='archive') archiveView.classList.remove('hidden'); if(name==='admin') adminView.classList.remove('hidden'); if(name==='info') infoView.classList.remove('hidden'); }
...
function renderToday(){ const today = new Date().toISOString().split('T')[0]; let content = state.messages.find(m=> m.scheduled === today && m.released) || state.messages.filter(m=>m.released).slice(-1)[0]; const container = todayContent; if(content){ container.innerHTML = <div class="msg" data-id="${content.id}">${buildMessageHTML(content)}${buildMessageControls(content)}</div>; } else { container.innerHTML = '<div class="muted-note">No content for today</div>'; } }
...
// build message html + controls
function buildMessageHTML(m){ return <div class="meta"><strong>${escapeHtml(m.title||'(No title)')}</strong> • ${new Date(m.dateAdded).toLocaleDateString()}</div><div style="white-space:pre-line">${escapeHtml(m.content)}</div>${m.audio?<div class="audio-section"><audio controls src="${m.audio}"></audio></div>:''}; }
...
function addCommentUI(mid){
  ...
  state.activity.push(${currentUser.name} commented on "${m.title||''}" at ${new Date().toLocaleDateString()});
  save(); renderAll();
...
document.addEventListener('click', function(e){
  ...
  if(ec){ const msgId = Number(ec.dataset.msg); const cid = Number(ec.dataset.editcomment); const m = state.messages.find(x=>x.id===msgId); if(!m) return; const c = m.comments.find(x=>x.id===cid); if(!c) return; const nt = prompt('Edit comment', c.text); if(nt!==null){ c.edits = c.edits || []; c.edits.push({ old: c.text, when: new Date().toISOString() }); c.text = nt; c.editedAt = new Date().toISOString(); state.activity.push(${c.by} edited a comment at ${new Date().toLocaleDateString()}); save(); renderAll(); } }
});
...
// delete
function deleteContent(id){ if(!confirm('Delete message?')) return; state.messages = state.messages.filter(x=>x.id!==id); state.activity.push(Admin deleted message at ${new Date().toLocaleDateString()}); save(); renderAll(); }
...
// renderAll & helpers
function renderAll(){ renderToday(); renderArchive(); renderAdminList(); renderActivity(); setTimeout(()=>startObserver(),200); }
function renderToday(){ const today = new Date().toISOString().split('T')[0]; let content = state.messages.find(m=> m.scheduled === today && m.released) || state.messages.filter(m=>m.released).slice(-1)[0]; todayContent.innerHTML = content ? <div class="msg" data-id="${content.id}">${buildMessageHTML(content)}${buildMessageControls(content)}</div> : '<div class="muted-note">No content for today</div>'; }
function renderArchive(){ archiveContent.innerHTML = state.messages.slice().reverse().map(m=><div class="msg" data-id="${m.id}">${buildMessageHTML(m)}${buildMessageControls(m)}</div>).join('') || '<div class="muted-note">No messages</div>'; }
function renderAdminList(){ contentList.innerHTML = state.messages.slice().reverse().map(m=><div style="padding:8px;background:#fff;border-radius:8px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center"><div><strong>${escapeHtml(m.title||'(No title)')}</strong><div class="small">${m.type} • ${new Date(m.dateAdded).toLocaleDateString()}</div></div><div><button class="btn ghost" data-edit="${m.id}">Edit</button><button class="btn ghost" data-delete="${m.id}">Delete</button></div></div>).join('') || '<div class="muted-note">No content scheduled yet.</div>'; document.querySelectorAll('[data-edit]').forEach(b=> b.onclick = ()=> editContent(Number(b.dataset.edit))); document.querySelectorAll('[data-delete]').forEach(b=> b.onclick = ()=> deleteContent(Number(b.dataset.delete))); }
function renderActivity(){ activityLog.innerHTML = (state.activity || []).slice().reverse().map(a=><div class="small">${escapeHtml(a)}</div>).join('') || '<div class="small">No activity</div>'; }
...
(function init(){
  ...
  // first user interaction will allow bg playback
  document.addEventListener('click', ()=>{ if(state.settings.audioData && !bgPlayer.src){ bgPlayer.src = state.settings.audioData; try{ bgPlayer.play(); }catch(e){} } }, { once:true });
})();
