...
// Final fixed script with quick fade transition; no welcome toast
...
// login modal fade helpers
function showLoginModal(){ const login=document.getElementById('loginModal'); login.classList.remove('hidden-opacity'); login.classList.add('visible-opacity'); }
function hideLoginModal(){ const login=document.getElementById('loginModal'); login.classList.remove('visible-opacity'); login.classList.add('hidden-opacity'); }
function showAppWithFade(){ const app=document.getElementById('app'); app.classList.remove('hidden-opacity'); app.classList.add('visible-opacity'); }
function hideApp(){ const app=document.getElementById('app'); app.classList.remove('visible-opacity'); app.classList.add('hidden-opacity'); }

// login flow
function openLogin(){ const login=document.getElementById('loginModal'); login.classList.remove('hidden'); // ensure visible in DOM
  // small delay to allow transition classes
  setTimeout(()=>{ showLoginModal(); },20);
}
function closeLogin(){ hideLoginModal(); setTimeout(()=>{ document.getElementById('loginModal').classList.add('hidden'); },300); }
function doLogin(){
...
function toggleAudio(){ const bg=document.getElementById('bgPlayer'); if(!bg.src){ return; } if(bg.paused){ bg.play().catch(()=>{}); document.getElementById('toggleAudioBtn').textContent='Pause'; } else { bg.pause(); document.getElementById('toggleAudioBtn').textContent='Play'; } }
...
// import/export/reset (kept simple)
...
function importJson(){ const ip=document.createElement('input'); ip.type='file'; ip.accept='application/json'; ip.onchange=(e)=>{ const f=e.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=(ev)=>{ try{ const obj=JSON.parse(ev.target.result); state=obj; saveState(); }catch(err){ alert('Invalid JSON'); } }; r.readAsText(f); }; ip.click(); }
function resetAll(){ if(confirm('Reset local data?')){ localStorage.removeItem(STORAGE_KEY); location.reload(); } }
...
function startReminderChecker(){ setInterval(()=>{ const now=new Date(); const due = state.reminders.filter(r=> !r.notified && new Date(r.when) <= now); due.forEach(r=>{ r.notified=true; vibrate(); state.activity.push('Reminder fired: '+r.title+' at '+new Date().toLocaleString()); }); saveState(); },10000); }
